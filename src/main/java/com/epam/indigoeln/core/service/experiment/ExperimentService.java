...
public class ExperimentService {
    ...
    public List<ExperimentTreeNodeDTO> getAllExperimentTreeNodes(String projectId, String notebookId, User user) {
        ...
        DBObject notebook = notebookCollection
        ...
        if (notebook.get("experiments") instanceof Iterable) {
            ...
            List<DBObject> experiments = new ArrayList<DBObject>();
            ...
            result.forEach(e -> {
                ...
                List<Object> components = experimentsWithComponents.get(e.getFullId());
    ...
    private void setValuesForExperimentTreeNodeDTO(ExperimentTreeNodeDTO experiment,
        ...
        if (components != null) {
            Optional reactionComponent = components.stream()
                    .filter(c -> ("reaction".equals(((BasicDBObject) c).get("name")))).findFirst();

            Optional reactionDetailsComponent = components.stream()
                    .filter(c -> ("reactionDetails".equals(((BasicDBObject) c).get("name")))).findFirst();

            Optional conceptDetailsComponent = components.stream()
    ...
    private void setSpecialFields(Object component, ExperimentTreeNodeDTO experiment) {
        if (component != null && experiment != null && ((BasicDBObject) component).get("content") != null) {
            Object titleObject = ((BasicDBObject) ((BasicDBObject) component)
                    .get("content")).get("title");
            if (titleObject != null) {
                experiment.setTitle(String.valueOf(titleObject));
            }
            Object therapeuticAreaObject = ((BasicDBObject) ((BasicDBObject) component)
    ...
    public ExperimentTreeNodeDTO getExperimentAsTreeNode(String projectId, String notebookId, String experimentId) {
        ...
        DBCollection experimentCollection = mongoTemplate.getCollection(Experiment.COLLECTION_NAME);
        DBCollection userCollection = mongoTemplate.getCollection(User.COLLECTION_NAME);
        DBObject experiment = experimentCollection
        ...
        DBObject user = userCollection.findOne(new BasicDBObject("_id", authorId));
        ...
        List components = getComponents(componentIds);
    ...
    private Map<Object, List<Object>> getExperimentsWithComponents(List<DBObject> experiments) {
        ...
        List allComponents = getComponents(allComponentIds);
        experimentsWithComponents.entrySet().forEach(entryObject -> {
            List<Object> ids = entryObject.getValue();
